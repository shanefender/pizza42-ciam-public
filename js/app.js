...
const emailNotVerified = async (fn, targetUrl) => {
  const isAuthenticated = await auth0.isAuthenticated();
  const user = await auth0.getUser();

  if (!user.email_verified) {
    showContentFromUrl("/verify-email");
  }
...
const callApi = async () => {
  try {
    ...
    responseElement.innerText = JSON.stringify(responseData, {}, 0);
...
window.onload = async () => {
  ...
  bodyElement.addEventListener("click", (e) => {
    ...
    } else if (e.target.getAttribute("id") === "order-or-verify") {
      e.preventDefault();
      emailNotVerified();
